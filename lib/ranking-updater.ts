...
export async function updateRankingCache() {
  ...
  try {
    ...
    const [totalRankings, weeklyRankings, newRankings] = await Promise.all([
      calculateTotalRanking(),
      calculateWeeklyRanking(),
      calculateNewRanking(),
    ]);

    console.log(`[Ranking Cache] Calculated ${totalRankings.length + weeklyRankings.length + newRankings.length} rankings`);
    ...
    return {
      success: true,
      totalCount: totalRankings.length,
      weeklyCount: weeklyRankings.length,
      newCount: newRankings.length,
      duration,
    };
  } catch (error) {
...
async function calculateWeeklyRanking() {
  ...
  return result.map(row => ({
    ...
    rating_score: row.rating_score,
    length_seconds: row.length_seconds,
    weekly_increase: row.weekly_increase,
  }));
}
